...
songplay_table_drop = "DROP TABLE IF EXISTS fact_tables.songplay;"
...
CREATE TABLE IF NOT EXISTS fact_tables.songplay (\
...
staging_events_copy = ("""\
copy staging_tables.staging_events from '{s3_url}'
...
staging_songs_copy = ("""
copy staging_tables.staging_songs from '{s3_url}' \
credentials 'aws_iam_role={aws_iam_role}' \
region 'us-west-2' \
JSON 'auto'; \
""").format(
...
INSERT INTO fact_tables.songplay (start_time, user_id, level, song_id, artist_id, session_id, location, user_agent) \
SELECT \
...
JOIN staging_tables.staging_songs ss \
...
FROM staging_tables.staging_events se \
WHERE se.ts = (\
SELECT max(se2.ts) FROM staging_tables.staging_events se2 WHERE se.userid = se2.userid);\
""")
...
FROM \
(SELECT DISTINCT(timestamp 'epoch' + se.ts/1000 * interval '1 second') as start_time FROM staging_tables.staging_events se);\
""")
...
create_schemas_queries = [
    fact_schema, dimension_schema, staging_schema
]

drop_schemas_queries = [
    fact_schema_drop, dimension_schema_drop, staging_schema_drop
]